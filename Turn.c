...
{
	if (lastAttempt == MAX_SCORE) {  // if strike - get two more rolls
		pTurn->isStrike = 1;
		...
		lastAttempt = getRandomScore(lastAttempt);
		pTurn->attempts[1] = lastAttempt;
		if (lastAttempt + pTurn->attempts[0] == MAX_SCORE) {  // if spare - get one more roll
			pTurn->isSpare = 1;
			lastAttempt = getRandomScore(0);
			pTurn->attempts[2] = lastAttempt;
		}
...
void printTurnAttempts(const Turn* pTurn)
{
	if (pTurn->isLastTurn) {  // special frame - frame 10
		...
		{
			printf("X ");
			if (pTurn->attempts[1] == MAX_SCORE) {  // second strike
				printf("X ");
...
{ 
	...
	if (pTurn1->isLastTurn)
		...
		if (pTurn1->isStrike) {  // score is 10 + two next attempts
			sum += 10;
...
int	saveTurnToFile(Turn* pTurn, FILE* fp)
{
	if (fwrite(pTurn, sizeof(Turn), 1, fp) != 1)
	{
		PRINT_ERROR_STR_RETURN_0("write turn")
	}
...
int	loadTurnFromFile(Turn* pTurn, FILE* fp)
{
	if (fread(pTurn, sizeof(Turn), 1, fp) != 1)
	{
		PRINT_ERROR_STR_RETURN_0("read turn")
	}
